...
- **CI**：每次提交到 `main` 或创建 Pull Request 时运行 Python 单元测试并构建 Docker 镜像。
- **Publish container**：提交到 `main`、推送 `v1.0.0` 形式的版本标签或者手动触发时，构建并发布 AMD64、ARM64 镜像到 GitHub Container Registry。
...
如果希望未登录用户也能拉取镜像，首次发布后可能需要在 GitHub Package 设置中手动将镜像改为公开。
...
流水线会按情况发布版本号、主次版本号、标签名和 `latest`，同时生成 SBOM 和构建来源证明。Dependabot 每月检查 Docker 和 GitHub Actions 依赖更新。
